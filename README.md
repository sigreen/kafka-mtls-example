...
OpenShift on Azure uses Azure Managed Disks:
...
- `managed-premium` ✅ - Premium SSD (default in config)
- `managed-csi` - CSI driver with premium disks
- `azurefile-csi` ❌ - Not recommended (use block storage)

**Performance by disk size (Premium SSD):**

| Disk Size | IOPS | Throughput | Monthly Cost (US East) |
|-----------|------|------------|------------------------|
| 128 GB | 500 | 100 MB/s | ~$18 |
| 256 GB | 1,100 | 125 MB/s | ~$36 |
| 512 GB | 2,300 | 150 MB/s | ~$72 |
| 1 TB | 5,000 | 200 MB/s | ~$144 |
...
storage:
  ...
  - id: 0
    ...
    size: 512Gi  # Or 1TB+ for production
    class: managed-premium
```

### Azure Availability Zones

Distribute Kafka across availability zones for high availability:
...
oc get pods -n kafka -o wide
```

### Azure Network Security

**NSG rules required:**
- Port 443: External client access
- Port 6443: OpenShift API
- All ports: Between worker nodes (internal)
...
### Alternative: LoadBalancer (Instead of Routes)

Use `kafka-cluster-mtls-loadbalancer.yaml` for Azure LoadBalancer:
...
**Internal LoadBalancer (private IP):**
```yaml
service.beta.kubernetes.io/azure-load-balancer-internal: "true"
service.beta.kubernetes.io/azure-load-balancer-internal-subnet: "kafka-subnet"
```
